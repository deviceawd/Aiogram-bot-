...
from config import REDIS_DB_FSM, REDIS_DB, REDIS_KEY_PREFIX_TRC, REDIS_URL
...
def _redis_key(tx_hash: str) -> str:
    return f"{REDIS_KEY_PREFIX_TRC}{tx_hash}"
...
def check_trc20_confirmation_task(tx_hash, target_address, username, chat_id, bot_id, lang):
    if not r:
        logger.error("Redis недоступен для check_trc20_confirmation_task")
        return
    ...
    try:
        ...
        logger.info(f"[tasks] check_trc20 result: {result}")
        ...
        if code in ("invalid_token", "no_transfers", "invalid_recipient"):
            ...
            if code == "invalid_token" or code == "no_transfers":
...
@celery_task_fallback
def periodic_check_pending_transactions_trc20():
    if not r:
    ...
    """
    try:
        keys = r.keys(f"{REDIS_KEY_PREFIX_TRC}*")
        for key in keys:
